  ...
  <div class="tags-nav">
    ...
    <div class="scroll-outer">
      <transition-group name="breadcrumb">
        <router-link tag="div" :class="['item',{'active':isActive(item)}]" :to="item.path" v-for="item in tagsList" :key="item.name">
          <span class="left-dot"></span>
          <span class="text">{{item.title}}</span>
          <div @click.stop="closeSelectedTag(item.name)" v-if="item.name != 'dashboard'" class="close-tag">
    ...
    <el-dropdown class="close-con" @command="handleCommand">
      <div>
...
export default {
  name: 'tagsNav',
  created() {
    this.getHomePage(this.permissionRouter);
  },
  computed: {
    ...mapGetters(['tagsList', 'permissionRouter'])
  },
  watch: {
    $route() {
      this.addTagsNav();
